"""Need to say:'flask_first_steps_2.0' is more laconic literary name and does not
   stand out much from it`s previous versions. Code became more neat, clean and less stinky.
   Integrated such technology as migrations and application can work from Docker container
   and has everything else necessary for that purpose."""
...
migrate = Migrate(app, db)  # migration initialization
...
def exchange(currency_name_1, currency_name_2):
    ...
    if (user1_balance >= amount1) and (exist_currency_2 >= cur2_needed):
        ...
        Currency.query.filter_by(cur_name=currency_name_2, cur_date=current_date).update(
            dict(available_amount=exist_currency_2 - cur2_needed))
        db.session.commit()
        Currency.query.filter_by(cur_name=currency_name_1, cur_date=current_date).update(
        ...
        db.session.commit()
        Account.query.filter_by(user_id=user_id, cur_name=currency_name_2).update(
